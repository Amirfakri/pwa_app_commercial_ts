...
const errorHandler = (err, req, res, next) => {
    ...
    if (err instanceof AppError_1.AppError && err.isOperational) {
        winston_1.logger.warn(`⚠️ Operational error: ${err.message}`, {
        ...
        winston_1.logger.error(`❌ Unhandled error: ${err.message}`, {
            stack: err.stack,
            path: req.path,
            method: req.method,
            ip: req.ip,
    ...
    if (err.name === 'SequelizeValidationError' || err.code === '23502') {
        res.status(400).json({
            success: false,

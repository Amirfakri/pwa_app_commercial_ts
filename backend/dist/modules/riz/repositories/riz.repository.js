...
exports.rizRepository = exports.RizRepository = void 0;
...
class RizRepository {
    ...
    async getUserInfo(userCode) {
        const { rows } = await this.pool.query('SELECT code, first_name, last_name, mobile_number FROM users WHERE code = $1', [userCode]);
        return rows[0] || null;
    }
    // آپلود ریزحساب با پشتیبانی از sort_order
    async uploadRizData(records, userCode) {
        if (records.length === 0)
            return 0;
    ...
    async getUserTransactions(userCode, limit = 50, offset = 0) {
        const { rows } = await this.pool.query(`
      SELECT 
    ...
    async getTotalTransactionsCount(userCode) {
        const { rows } = await this.pool.query('SELECT COUNT(*) as total FROM riz WHERE user_code = $1', [userCode]);
...
exports.RizRepository = RizRepository;
exports.rizRepository = new RizRepository();

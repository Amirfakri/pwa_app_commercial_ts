...
class TimerController {
    ...
    async getAllTimers(req, res) {
        try {
            ...
            const result = await timer_service_1.timerService.getAllTimers(page, limit, status);
            // دریافت اطلاعات کاربران برای هر تایمر
            const formattedTimers = await Promise.all(result.timers.map(async (timer) => {
                // دریافت اطلاعات کاربر از دیتابیس
                const userResult = await postgres_1.pool.query(`SELECT first_name, last_name, code FROM users WHERE id = $1`, [timer.user_id]);
    ...
    async getTimerStats(req, res) {
        try {
            ...
            const stats = await timer_service_1.timerService.getTimerStats();
            res.json({
                success: true,
                data: stats
            });
    ...
    async deleteTimer(req, res) {
        try {
            ...
            const deleted = await timer_service_1.timerService.deleteTimer(parseInt(id));
            if (!deleted) {
                res.status(404).json({ success: false, error: 'تایمر یافت نشد' });
                return;
            }
    ...
    async deleteTimerByTransaction(req, res) {
        try {
            const { transactionId } = req.params;
            if (!req.user?.isAdmin) {
                res.status(403).json({ success: false, error: 'دسترسی غیرمجاز: فقط ادمین‌ها' });

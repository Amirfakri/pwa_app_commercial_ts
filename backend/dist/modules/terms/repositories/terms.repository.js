...
class TermsRepository {
    ...
    async create(content, adminId) {
       ...
       RETURNING id, content, version, is_active, created_by, created_at, updated_at`, [content, newVersion, adminId]);
        return rows[0];
    }
    ...
    async getUserAcceptance(userId) {
        const { rows } = await this.pool.query(`SELECT user_id, terms_version, accepted_at 

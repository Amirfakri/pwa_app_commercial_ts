...
class AdminRepository {
    ...
    async findAll() {
        ...
        const { rows } = await this.pool.query(`SELECT id, mobile_number, first_name, last_name, is_admin as is_main_admin, 
    ...
    async findById(id) {
        const { rows } = await this.pool.query(`SELECT id, mobile_number, first_name, last_name, is_admin as is_main_admin, 
              NOT is_blocked as is_active, created_at
       FROM users WHERE id = $1 AND is_admin = true`, [id]);
        if (rows.length === 0)
            return null;
        const admin = rows[0];
    ...
    async findByMobile(mobile) {
        const { rows } = await this.pool.query(`SELECT id, mobile_number, first_name, last_name, is_admin as is_main_admin, 
              NOT is_blocked as is_active, created_at
       FROM users WHERE mobile_number = $1 AND is_admin = true`, [mobile]);
        if (rows.length === 0)
            return null;
        const admin = rows[0];
        admin.full_name = `${admin.first_name || ''} ${admin.last_name || ''}`.trim() || 'ادمین';
    ...
    async create(data) {
       ...
       RETURNING id, mobile_number, first_name, last_name, is_admin as is_main_admin, 
                 false as is_blocked, created_at`, [data.mobile_number, data.first_name, data.last_name, code, 5]);
        const admin = rows[0];
        admin.full_name = `${admin.first_name || ''} ${admin.last_name || ''}`.trim() || 'ادمین';
    ...
    async update(id, is_active) {
        // فعال/غیرفعال کردن ادمین
        const { rows } = await this.pool.query(`UPDATE users SET is_blocked = $1, updated_at = NOW() 
       WHERE id = $2 AND is_admin = true RETURNING *`, [!is_active, id]);
        if (rows.length === 0)
            return null;
        const admin = rows[0];
        admin.full_name = `${admin.first_name || ''} ${admin.last_name || ''}`.trim() || 'ادمین';
    ...
    async delete(id) {
        const { rowCount } = await this.pool.query('DELETE FROM users WHERE id = $1 AND is_admin = true AND is_admin = false', [id]);
        return (rowCount || 0) > 0;
    }
    async isMainAdmin(id) {
        const { rows } = await this.pool.query('SELECT is_admin as is_main_admin FROM users WHERE id = $1 AND is_admin = true', [id]);

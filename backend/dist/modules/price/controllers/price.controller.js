...
exports.priceController = exports.PriceController = void 0;
...
class PriceController {
    ...
    getTodayPrice = async (req, res) => {
        try {
            ...
            const price = await price_service_1.priceService.getTodayPrice(product_code, userId, isAdmin);
            if (!price) {
                res.status(200).json({
    ...
    getAllTodayPrices = async (req, res) => {
        try {
            const userId = req.user.id;
            const isAdmin = req.user.isAdmin;
            const prices = await price_service_1.priceService.getAllTodayPrices(userId, isAdmin);
    ...
    createPrice = async (req, res) => {
        try {
            ...
            const price = await price_service_1.priceService.createOrUpdatePrice(priceData, req.user.id);
    ...
    updatePrice = async (req, res) => {
        try {
            ...
            const price = await price_service_1.priceService.updatePriceById(parseInt(id), priceData, req.user.id);
            if (!price) {
    ...
    getProductsWithPrices = async (req, res) => {
        try {
            const userId = req.user.id;
            const isAdmin = req.user.isAdmin;
            const products = await price_service_1.priceService.getProductsWithPrices(userId, isAdmin);
    ...
    getPriceHistory = async (req, res) => {
        try {
            ...
            const history = await price_service_1.priceService.getPriceHistory(product_code, start_date, end_date);
            res.json({
                success: true,
                data: history,
    ...
    updateUserOffsets = async (req, res) => {
        try {
            const { user_id } = req.params;
            const { melted_offset, coin_offset } = req.body;
            const offsets = await offset_service_1.offsetService.updateUserOffsets(parseInt(user_id), melted_offset, coin_offset);
            const io = req.app.get('io');
            if (io) {
    ...
    externalPriceWebhook = async (req, res) => {
        try {
    ...
    getExternalPrice = async (req, res) => {
        try {
            ...
            const price = await price_repository_1.priceRepository.getLatestPrice(product_code);
            if (!price) {
                res.status(404).json({
                    success: false,
    ...
    getAllExternalPrices = async (req, res) => {
        try {
            const userId = req.user.id;
            const isAdmin = req.user.isAdmin;
    ...
    broadcastPriceToUsers = async (io, price, source = 'admin') => {
        try {
            ...
            for (const room of userRooms) {
                ...
                if (isNaN(userId))
                    continue;
                const offsets = await offset_service_1.offsetService.getUserOffsets(userId, false);
                const offset = price.product_code.startsWith('AB_')
...
exports.priceController = new PriceController();

...
import { IPrice, ICreatePriceInput, IUpdatePriceInput } from '../types/price.types';
...
export class PriceRepository {
  ...
  async getTodayPrice(productCode: string, today: string): Promise<IPrice | null> {
  ...
  async getLatestPrice(productCode: string): Promise<IPrice | null> {
    const { rows } = await this.pool.query(
      `SELECT id, product_code, buy_price, sell_price, 
              is_visible_buy, is_visible_sell, display_name, 
              price_date, created_at, updated_at
  ...
  async getAllTodayPrices(today: string): Promise<IPrice[]> {
    const { rows } = await this.pool.query(
      `SELECT id, product_code, buy_price, sell_price, 
              is_visible_buy, is_visible_sell, display_name, 
              price_date, created_at, updated_at
  ...
  async createPrice(data: ICreatePriceInput, priceDate: string): Promise<IPrice> {
    const { rows } = await this.pool.query(
      `INSERT INTO prices (
  ...
  async upsertPrice(data: ICreatePriceInput, priceDate: string): Promise<IPrice> {
  ...
  async updatePriceById(id: number, data: IUpdatePriceInput): Promise<IPrice | null> {
    const updates: string[] = [];
    const values: any[] = [];
    let paramIndex = 1;

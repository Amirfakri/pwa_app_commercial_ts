...
export class ProductService {
  ...
  async getAllProducts() {
    const melted = await this.pool.query(
      'SELECT *, \'melted\' as type FROM melted_products ORDER BY display_order'
    );
    const coins = await this.pool.query(
      'SELECT *, \'coin\' as type FROM coins ORDER BY display_order'
    );
  ...
  async getProductByCode(code: string) {
    ...
    if (code.startsWith('AB_')) {
      result = await this.pool.query(
        'SELECT *, \'melted\' as type FROM melted_products WHERE code = $1',
        [code]
      );
    } else {
      result = await this.pool.query(
        'SELECT *, \'coin\' as type FROM coins WHERE code = $1',
        [code]
      );
    }

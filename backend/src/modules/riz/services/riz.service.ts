...
export class RizService {
  // دریافت خلاصه همه مشتریان
  async getCustomerSummaries(): Promise<any[]> {
    return rizRepository.getCustomerSummaries();
  }

  // دریافت لیست کاربران برای جستجو
  async getAllUsers(): Promise<any[]> {
    return rizRepository.getAllUsersForSearch();
  }

  // دریافت مانده حساب یک کاربر
  async getUserBalance(userCode: string): Promise<any> {
    const balance = await rizRepository.getUserBalance(userCode);
    const userInfo = await rizRepository.getUserInfo(userCode);
    ...
    return {
      ...
      total_transactions: await rizRepository.getTotalTransactionsCount(userCode)
    };
  }

  // دریافت تراکنش‌های یک کاربر (از آخر به اول)
  async getUserTransactions(userCode: string, page: number = 1, limit: number = 50): Promise<{
    transactions: any[];
    pagination: {
      page: number;
      limit: number;
    ...
    const transactions = await rizRepository.getUserTransactions(userCode, limit, offset);
    const total = await rizRepository.getTotalTransactionsCount(userCode);
  ...
  private transformRizData(record: any, index: number): any {
  ...
  // آپلود ریزحساب
  async uploadRizData(rizData: any[], userCode: string): Promise<{ inserted: number; user_code: string }> {
    ...
    const inserted = await rizRepository.uploadRizData(records, userCode);
    return { inserted, user_code: userCode };
  }

  // حذف یک رکورد
  async deleteRecord(id: number): Promise<boolean> {
    const record = await rizRepository.findRecordById(id);
    if (!record) return false;
    return rizRepository.deleteRecord(id);
  }

  // حذف همه تراکنش‌های یک کاربر
  async deleteAllUserRecords(userCode: string): Promise<number> {
    return rizRepository.deleteAllUserRecords(userCode);
  }
}

export const rizService = new RizService();
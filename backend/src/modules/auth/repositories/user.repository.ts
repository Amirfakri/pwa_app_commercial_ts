...
export class UserRepository {
  ...
  async findAll(filters: { search?: string; is_blocked?: boolean; page?: number; limit?: number }): Promise<{ users: IUser[]; total: number }> {
  ...
  async create(data: { mobile_number: string; first_name?: string; last_name?: string; code?: string; is_admin?: boolean }): Promise<IUser> {
    ...
    const { rows } = await this.pool.query(
      `INSERT INTO users (mobile_number, first_name, last_name, code, is_admin, device_limit)
       ...
       RETURNING id, mobile_number, first_name, last_name, code, is_admin, is_blocked,
                 device_limit, melted_price_offset, coin_price_offset, created_at`,
      [data.mobile_number, data.first_name || 'کاربر', data.last_name || 'جدید', code, isAdmin, deviceLimit]
    );
    const user = rows[0];
    user.full_name = `${user.first_name || ''} ${user.last_name || ''}`.trim() || user.code;
    return user;
  ...
  async updateOffsets(id: number, meltedOffset: number, coinOffset: number): Promise<IUser | null> {
    const { rows } = await this.pool.query(
      `UPDATE users SET melted_price_offset = $1, coin_price_offset = $2, updated_at = NOW()
  ...
  async createUserFromRegistration(registration: any): Promise<IUser> {
  ...
  async getPendingRegistrations(): Promise<any[]> {
    const { rows } = await this.pool.query(
      `SELECT id, mobile_number, first_name, last_name, national_code, created_at
       FROM user_registrations WHERE status = 'pending' ORDER BY created_at DESC`
    );
    return rows;
  }

  async approveRegistration(id: number, adminId: number): Promise<any> {
    const { rows } = await this.pool.query(
      `UPDATE user_registrations SET status = 'approved', approved_by = $1, approved_at = NOW()
  ...
  async rejectRegistration(id: number, adminId: number): Promise<any> {
    const { rows } = await this.pool.query(
      `UPDATE user_registrations SET status = 'rejected', approved_by = $1, approved_at = NOW()
       WHERE id = $2 AND status = 'pending' RETURNING *`,
      [adminId, id]
    );
    return rows[0] || null;

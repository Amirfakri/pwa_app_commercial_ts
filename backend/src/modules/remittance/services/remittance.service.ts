...
import { remittanceRepository } from '../repositories/remittance.repository';
...
export class RemittanceService {
  async createRemittance(
    ...
    const remittance = await remittanceRepository.create({
      user_id: userId,
      type: input.type,
      weight: input.weight,
      amount: input.amount,
  ...
  async updateRemittanceStatus(id: number, status: string, req?: any): Promise<any | null> {
    const validStatuses = ['در حال بررسی', 'تأییدشده', 'ردشده', 'تحویل شده'];
    if (!validStatuses.includes(status)) {
      throw new Error('وضعیت نامعتبر است');
    }

    const remittance = await remittanceRepository.updateStatus(id, status);
    if (!remittance) {
      return null;
    }
  ...
  async getUserInfo(userId: number): Promise<any> {
    const userInfo = await remittanceRepository.getUserInfo(userId);
    if (userInfo) {
      return {
        ...userInfo,
  ...
  async deleteRemittance(id: number): Promise<boolean> {
    return remittanceRepository.delete(id);
  }
  
  async getUserRemittances(userId: number, page: number = 1, limit: number = 50): Promise<any> {
    const offset = (page - 1) * limit;
    const remittances = await remittanceRepository.findByUserId(userId, limit, offset);
    const total = await remittanceRepository.countByUserId(userId);
  ...
  async getAllRemittances(filters: { status?: string; page?: number; limit?: number; search?: string }): Promise<any> {
    const result = await remittanceRepository.findAll(filters);
    
    const formattedRemittances = result.remittances.map((r: any) => ({
      id: r.id,
      user_id: r.user_id,
  ...
  async getRemittanceStats(): Promise<any> {
    const stats = await remittanceRepository.getStats();
    return stats;
  }

  async getRecentPendingRemittances(limit: number = 50): Promise<any[]> {
    const pending = await remittanceRepository.getRecentPending(limit);
  ...
  async getTodayRemittances(status?: string, page: number = 1, limit: number = 50): Promise<any> {
    const result = await remittanceRepository.getTodayRemittances(status, page, limit);

...
import { INotification, ICreateNotificationInput, IUserNotification, NotificationAudience } from '../types/notification.types';
...
export class NotificationRepository {
  ...
  async findUserNotifications(userId: number, filters: { page?: number; limit?: number; is_read?: boolean }): Promise<{ notifications: any[]; total: number }> {
  ...
  async findActive(userId?: number): Promise<INotification[]> {
    let query = `
  ...
  async findAll(filters: { is_active?: boolean; page?: number; limit?: number; audience?: NotificationAudience; user_id?: number }): Promise<{ notifications: INotification[]; total: number }> {
  ...
  async create(data: ICreateNotificationInput, adminId: number): Promise<INotification> {
    const { rows } = await this.pool.query(
  ...
  async createUserNotificationRecord(notificationId: number, userId: number): Promise<void> {
    await this.pool.query(
      `INSERT INTO user_notifications (notification_id, user_id, is_read, read_at, created_at)
  ...
  async update(id: number, data: Partial<ICreateNotificationInput>): Promise<INotification | null> {

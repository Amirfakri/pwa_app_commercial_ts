...
export class UserRepository {
  ...
  async getAllUsers(filters: { search?: string; is_blocked?: boolean; page?: number; limit?: number }): Promise<{ users: IUser[]; total: number }> {
  ...
  async updateUser(id: number, data: any): Promise<IUser | null> {
  ...
  async blockUser(id: number, blocked: boolean): Promise<boolean> {
  ...
  async createUserFromRegistration(registration: IUserRegistration): Promise<IUser> {
    ...
    const { rows } = await this.pool.query(
      `INSERT INTO users (mobile_number, first_name, last_name, code, device_limit)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [registration.mobile_number, registration.first_name, registration.last_name, userCode, 1]
    );
    return rows[0];
  }
}
